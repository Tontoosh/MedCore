      ...
      <div className="p-8 max-w-5xl">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-slate-500 text-sm mt-0.5">Нийт {mockPatients.length} өвчтөн</p>
          </div>
          <Link
            href="/patients/new"
        ...
        <div className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden">
            ...
            <tbody className="divide-y divide-slate-50">
              {filtered.map((p) => (
                <tr key={p.id} className="hover:bg-slate-50 transition group">
                  ...
                  <td className="px-6 py-4">
                    <span className="font-mono text-xs text-slate-600 bg-slate-100 px-2 py-1 rounded">{p.medicalRecordNo}</span>
                  </td>
                  <td className="px-6 py-4">
                    <span className="text-sm text-slate-700">{p.age} нас</span>
                    <span className="text-slate-400 text-xs ml-1">· {p.gender === 'male' ? 'Эр' : 'Эм'}</span>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4 text-right">
                    <Link
                      href={`/patients/${p.id}`}
                      className="text-blue-600 hover:text-blue-700 text-sm font-medium opacity-0 group-hover:opacity-100 transition"
                    >
                      Харах →
                    </Link>
                  </td>
          ...
          {filtered.length === 0 && (
            <div className="text-center py-12 text-slate-400 text-sm">Хайлтад тохирох өвчтөн олдсонгүй</div>
          )}
        </div>
      </div>
    </AppShell>
